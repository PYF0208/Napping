...
namespace Napping_PJ.Areas.Admin.Controllers
{
	...
	public class HotelsController : Controller
	{
			...
			[FromBody] HotelsViewModel hotel)
		{
			...
			h.Name.Contains(hotel.Name) ||
			h.Star==hotel.Star ||
			h.Image.Contains(hotel.Image) ||
			h.ContactName.Contains(hotel.ContactName) ||
			h.Phone.Contains(hotel.Phone) ||
			h.Email.Contains(hotel.Email) ||
			h.City.Contains(hotel.City) ||
			h.Region.Contains(hotel.Region) ||
			h.Address.Contains(hotel.Address)).Select(h => new HotelsViewModel
			{
				HotelId = h.HotelId,
				Name = h.Name,
				Star = h.Star,
